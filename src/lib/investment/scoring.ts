...
function scoreFinancialHealth(metrics: FinancialMetrics) {
...
function scoreFiveYearReturn(metrics: FinancialMetrics) {
...
function getReturnMomentumFloor(metrics: FinancialMetrics, riskControlScore: number) {
  const fiveYearReturn = metrics.fiveYearReturnPercent;
...
function getMissingCoreMetricCount(metrics: FinancialMetrics) {
  return [
    metrics.marketCap <= 0,
...
function getCompletenessScoreCap(missingCoreMetricCount: number) {
...
function getReturnAdjustment(metrics: FinancialMetrics, riskControlScore: number) {
  const fiveYearReturn = metrics.fiveYearReturnPercent;
...
function scoreValuation(metrics: FinancialMetrics) {
...
function scoreRisk(metrics: FinancialMetrics, news: NewsItem[]) {
...
function hasSourceType(sources: SourceReference[], sourceType: SourceReference["sourceType"]) {
  return sources.some((source) => source.sourceType === sourceType);
}

function getSourceCoverageScore(sources: SourceReference[]) {
  const hasOfficial = hasSourceType(sources, "Official");
  const trustedThirdPartyCount = sources.filter(
    (source) => source.sourceType === "Trusted Third Party" && source.credibility === "High",
  ).length;
...
function getDataQualityScore(status: DataQualityStatus, sources: SourceReference[]) {
...
export function scoreInvestment(
  ...
  const categories: ScoreCategory[] = [
    ...
    {
      label: metrics.returnPeriodLabel ?? "Last 5 Years Return",
  ...
  const riskControlScore = categories.find((category) => category.label === "Risk Control")?.score ?? 0;
  ...
  const totalScore = Math.round(
    clamp(Math.min(Math.max(weightedScore + returnAdjustment, returnMomentumFloor), completenessScoreCap)),
  );
  ...
  const confidenceNotes = [
    ...
    ...(metrics.dataWarnings ?? []).map((warning) => `Data warning: ${warning}`),
...
export function scoreGuidedBotProject(): ScoreBreakdown {
  return {
    categories: [
      {
...
export function scoreUnverifiedCompany(): ScoreBreakdown {
  const categories: ScoreCategory[] = [
    {
      label: "Financial Health",
      score: 0,
