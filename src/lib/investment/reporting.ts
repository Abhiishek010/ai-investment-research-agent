...
function missingCoreMetricCount(metrics: FinancialMetrics) {
  return [
    metrics.marketCap <= 0,
...
function hasInsufficientVerifiedFinancials(metrics: FinancialMetrics) {
  return missingCoreMetricCount(metrics) >= 2;
}

export function getVerdict(totalScore: number, metrics?: FinancialMetrics): InvestmentVerdict {
...
function getVerdictArticle(verdict: InvestmentVerdict) {
  return verdict === "Invest" ? "an" : "a";
}

function getReturnLabel(metrics: FinancialMetrics) {
  return (metrics.returnPeriodLabel ?? "Last 5 Years Return").toLowerCase();
}

function hasCoreMetricDataGap(metrics: FinancialMetrics) {
  return hasInsufficientVerifiedFinancials(metrics);
}

function formatMetricPercent(value: number, metrics: FinancialMetrics) {
  return hasCoreMetricDataGap(metrics) && value === 0 ? "not available" : `${value.toFixed(1)}%`;
}

function formatMetricNumber(value: number, metrics: FinancialMetrics) {
  return hasCoreMetricDataGap(metrics) && value === 0 ? "not available" : value.toFixed(1);
}

function describeReturn(metrics: FinancialMetrics) {
...
function describeValuation(metrics: FinancialMetrics) {
...
function buildStrengths(metrics: FinancialMetrics, news: NewsItem[], company?: CompanyProfile) {
  ...
  if (metrics.fiveYearReturnPercent >= 50) {
    strengths.push(`The stock has created meaningful shareholder returns on ${metrics.returnPeriodLabel ?? "the available return period"}.`);
  }
...
function buildRisks(company: CompanyProfile, metrics: FinancialMetrics, news: NewsItem[]) {
  if (company.ticker === "INVESTABLEBOT") {
    return [
  ...
  if (metrics.fiveYearReturnPercent < -20) {
    risks.push(`The stock has lost significant value on ${metrics.returnPeriodLabel ?? "the available return period"}.`);
  }
...
function buildSummary(
  ...
  if (company.marketStatus === "Subsidiary/Brand") {
    if (hasCoreMetricDataGap(metrics)) {
      return `${company.resolvedFrom ?? company.name} is not separately listed, so the bot resolved it to parent company ${company.parentCompany ?? company.name} (${company.ticker}). The bot still needs verified parent-company fundamentals before giving an investment verdict. Return data alone is not enough.`;
    }

    return `${company.resolvedFrom ?? company.name} is not separately listed, so the bot resolved it to parent company ${company.parentCompany ?? company.name} (${company.ticker}). The parent stock receives ${getVerdictArticle(verdict)} ${verdict.toLowerCase()} verdict with a score of ${score.totalScore}/100, using parent-company financials and market data.`;
  }
  ...
  return `${company.name} receives ${getVerdictArticle(verdict)} ${verdict.toLowerCase()} verdict with a score of ${score.totalScore}/100. The result is driven by ${metrics.revenueGrowthPercent.toFixed(
    1,
  )}% revenue growth, ${metrics.profitMarginPercent.toFixed(
    1,
  )}% profit margin, ${metrics.debtToEquity.toFixed(2)} debt-to-equity, a P/E ratio of ${metrics.peRatio.toFixed(
    1,
  )}, and ${metrics.fiveYearReturnPercent.toFixed(1)}% ${getReturnLabel(metrics)}, where ${describeValuation(metrics)} and ${describeReturn(metrics)}.`;
}
...
export function buildResearchReport(input: {
  ...
  return {
    ...
    methodology:
      "The agent resolves the company, collects structured financial, return, and news evidence, checks official sources against trusted third-party sources, calculates a weighted score, and then explains the verdict in plain language.",
